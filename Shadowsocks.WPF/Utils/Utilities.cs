...
public static class Utilities
{
    ...
    public static string GetTempPath()
    {
        if (_tempPath == null)
        {
            var isPortableMode = false; // TODO: fix --profile-directory
            try
            {
                if (isPortableMode)
                {
    ...
    public static string ScanQrCodeFromScreen()
    {
        ...
        {
            var marginLeft = (int)((double)bmp.Width * i / 2.5 / maxTry);
            var marginTop = (int)((double)bmp.Height * i / 2.5 / maxTry);
            var cropRect = new Rectangle(marginLeft, marginTop, bmp.Width - marginLeft * 2, bmp.Height - marginTop * 2);
            var target = new Bitmap((int)screenWidth, (int)screenHeight);

            var imageScale = screenWidth / cropRect.Width;
            using (var graphics = Graphics.FromImage(target))
                graphics.DrawImage(bmp, new Rectangle(0, 0, target.Width, target.Height), cropRect, GraphicsUnit.Pixel);
