...
public class PrivoxyRunner : IEnableLogger
{
    ...
    public void Start(NetSettings netSettings)
    {
        if (_process == null)
        {
            ...
            foreach (var process in existingPrivoxy.Where(IsChildProcess))
            {
                KillProcess(process);
            }
            var privoxyConfig = Properties.Resources.privoxy_conf;
    ...
    private int GetFreePort(NetSettings netSettings)
    {
        ...
        try
        {
            ...
            var l = new TcpListener(IPAddress.Parse(netSettings.Socks5ListeningAddress), 0);
            l.Start();
