...
public class WinINet
{
    ...
    private const string SETTING_FILE = "wininet-setting.json";
    private static readonly WinINetSetting _initialSetting;
    ...
    static WinINet()
    {
        try
        {
            _initialSetting = Query();
        }
        catch (DllNotFoundException)
        {
            Operational = false;
            // Not on windows
            _logger.Info("You are not running on Windows platform, system proxy will disable");
        }
        catch (Win32Exception we)
        {
            if (we.NativeErrorCode == 12178)
            {
                _logger.Warn("WPAD service is not running, system proxy will disable");
                // WPAD not running
            }
            else
            {
    ...
    {
        List<InternetPerConnectionOption> options =
        [
    ...
    public static void Restore() => Set(_initialSetting);
    ...
    public static void Reset() => Set(new WinINetSetting());
    ...
    public static WinINetSetting Query()
    {
        ...
        List<InternetPerConnectionOption> outOptions = [];
