...
public class ServerSharingViewModel : ReactiveObject
{
    ...
    private void UpdateUrlAndImage()
    {
        ...
        // paint bitmap
        var blockSize = Math.Max(1024 / byteMatrix.Height, 1);
        var drawArea = new Bitmap((byteMatrix.Width * blockSize), (byteMatrix.Height * blockSize));
        using var graphics = Graphics.FromImage(drawArea);
        graphics.Clear(Color.White);
        using var solidBrush = new SolidBrush(Color.Black);
        for (var row = 0; row < byteMatrix.Width; row++)
        {
            for (var column = 0; column < byteMatrix.Height; column++)
            {
