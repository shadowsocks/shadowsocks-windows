...
namespace Shadowsocks.Controller
{
    ...
    public sealed class AvailabilityStatistics : IDisposable
    {
        ...
        private void ping_Completed(object sender, MyPing.CompletedEventArgs e)
        {
            ...
            logger.Debug($"Ping {server.ToString()} {e.RoundtripTime.Count} times, {(100 - record.PackageLoss * 100)}% packages loss, min {record.MinResponse} ms, max {record.MaxResponse} ms, avg {record.AverageResponse} ms");
        ...
        class MyPing
        {
            ...
            private void ICMPTest(int delay, object userstate)
            {
                ...
                {
                    logger.Error($"An exception occured while eveluating {server.ToString()}");
                    logger.LogUsefulException(e);
