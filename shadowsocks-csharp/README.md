...
The original from clowwindy42@gmail.com have something I really dont like:
1. It uses a external executible file privoxy.exe, and it has no use whatsoever, the path is proxy:
	1080->proxy:8123@privoxy->socks5:1080@TCPRelay->extern socks5 server, so you see it's big waste 
	of both programming time and computer time.
2. The external program privoxy.exe is unzipped at runtime to disk and the run from local. Since nobody
	mentioning that they checked its source, so it is a security hazard, which I hate and can not allow.
