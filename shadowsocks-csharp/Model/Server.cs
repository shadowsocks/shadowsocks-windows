...
namespace Shadowsocks.Model
{
    ...
    public class Server
    {
        ...
        public string FriendlyName()
        {
            ...
            if (country.IsNullOrEmpty() || !(country[0] >= 'A' && country[0] <= 'Z') || !(country[1] >= 'A' && country[1] <= 'Z'))
            {
                GeoIP.GeoIP geoip = new GeoIP.GeoIP();
                country = geoip.GetCityCode(server);
            }

            string serverStr = '[' + country.ToUpper() + "] " + $"{FormatHostName(server)}:{server_port}";
