...
internal class Socks5Client(NetworkCredential credential = null) : IStreamClient
{
    public async Task Connect(EndPoint destination, IDuplexPipe client, IDuplexPipe server)
    {
        ...
        {
            Auth = credential == null ? new[] { Socks5Message.AuthNone } : new[] { Socks5Message.AuthNone, Socks5Message.AuthUserPass }
        });
