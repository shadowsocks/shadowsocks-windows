...
namespace Shadowsocks.Services
{
    class PrivoxyRunner
    {
        ...
        static PrivoxyRunner()
        {
            try
            {
                Utils.UncompressFile(Utils.GetTempPath("ss_privoxy.exe"), Resources.privoxy_exe);
                Utils.UncompressFile(Utils.GetTempPath("mgwz.dll"), Resources.mgwz_dll);
            }
            catch (IOException e)
            {
        ...
        public void Start(Configuration configuration)
        {
            ...
            if (_process == null)
            {
                ...
                File.WriteAllBytes(Utils.GetTempPath("privoxy.conf"), Encoding.UTF8.GetBytes(polipoConfig));

                _process = new Process();
                // Configure the process using the StartInfo properties.
                _process.StartInfo.FileName = "ss_privoxy.exe";
                _process.StartInfo.Arguments = "privoxy.conf";
                _process.StartInfo.WorkingDirectory = Utils.TempPath;
