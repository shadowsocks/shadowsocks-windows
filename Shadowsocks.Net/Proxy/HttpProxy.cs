...
public class HttpProxy : IProxy, IEnableLogger
{
    ...
    private static readonly Regex _httpRespondHeaderRegex = new(@"^(HTTP/1\.\d) (\d{3}) (.+)$", RegexOptions.Compiled);
    ...
    private bool OnLineRead(string line, object state)
    {
        ...
        {
            var m = _httpRespondHeaderRegex.Match(line);
            if (m.Success)
            {
    ...
    public async Task ConnectRemoteAsync(EndPoint destEndPoint, CancellationToken token = default)
    {
        ...
        var authInfo = string.Empty;
        if (_auth != null)
        {
            var authKey = Convert.ToBase64String(Encoding.UTF8.GetBytes(_auth.UserName + ":" + _auth.Password));
            authInfo = string.Format(PROXY_AUTH_TEMPLATE, authKey);
        }
        var request = string.Format(HTTP_CONNECT_TEMPLATE, destEndPoint, authInfo);
        ...
        // start line read
        var reader = new LineReader(_remote, OnLineRead, (e, _) => throw e, (_1, _2, _3, _4) => { }, Encoding.UTF8, HTTP_CRLF, 1024, null);
