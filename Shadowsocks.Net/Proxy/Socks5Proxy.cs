...
public class Socks5Proxy : IProxy
{
    ...
    private const int SOCKS5_PKT_MAX_SIZE = 4 + 16 + 2;
    private readonly byte[] _receiveBuffer = new byte[SOCKS5_PKT_MAX_SIZE];
    ...
    public async Task ConnectProxyAsync(EndPoint remoteEP, NetworkCredential auth = null, CancellationToken token = default)
    {
        ...
        if (await _remote.ReceiveAsync(_receiveBuffer.AsMemory(0, 2), SocketFlags.None, token) != 2)
        {
