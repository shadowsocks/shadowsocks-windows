...
public abstract class AEADCrypto : CryptoBase, IEnableLogger
{
    ...
    private static readonly byte[] _infoBytes = Encoding.ASCII.GetBytes(INFO);
    ...
    public virtual void InitCipher(byte[] salt, bool isEncrypt)
    {
        ...
        HKDF.DeriveKey(HashAlgorithmName.SHA1, masterKey, sessionKey, salt, _infoBytes);
    ...
    {
        // push data
        Span<byte> tmp = _buffer.AsSpan(0, plain.Length + _bufPtr);
        plain.CopyTo(tmp.Slice(_bufPtr));
    ...
    public override int Decrypt(Span<byte> plain, ReadOnlySpan<byte> cipher)
    {
        ...
        Span<byte> tmp = _buffer.AsSpan(0, cipher.Length + _bufPtr);
        cipher.CopyTo(tmp.Slice(_bufPtr));
        int bufSize = tmp.Length;
    ...
    private int ChunkEncrypt(ReadOnlySpan<byte> plain, Span<byte> cipher)
    {
        ...
        var lenbuf = BitConverter.GetBytes((ushort)IPAddress.HostToNetworkOrder((short)plain.Length));
        var cipherLenSize = CipherEncrypt(lenbuf, cipher);
        nonce.Increment();
        var cipherDataSize = CipherEncrypt(plain, cipher.Slice(cipherLenSize));
        nonce.Increment();
    ...
    private int ChunkDecrypt(Span<byte> plain, ReadOnlySpan<byte> cipher)
    {
        ...
        var len = CipherDecrypt(plain, cipher.Slice(ChunkLengthBytes + tagLen, chunkLength + tagLen));
        nonce.Increment();
