...
public class StreamCryptoBaseCrypto(string method, string password) : StreamCrypto(method, password)
{
    ...
    protected override int CipherEncrypt(ReadOnlySpan<byte> plain, Span<byte> cipher) => CipherUpdate(plain, cipher);

    protected override int CipherDecrypt(Span<byte> plain, ReadOnlySpan<byte> cipher) => CipherUpdate(cipher, plain);
